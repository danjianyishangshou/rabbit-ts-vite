...
const delCartItem = async (SkuIds: string[]) => {
    ...
    cart.deleteCartList(SkuIds).catch(() => { })
}
const changeSelected = async (skuId: string, selected: any) => {
    await cart.updateCart(skuId, { selected })
}
const changeCount = async (skuId: string, count: any) => {
        ...
        <div class="container">
            ...
            <div class="cart">
                <table>
                        ...
                        <tr>
                            <th width="120">
                                <XtxCheckbox :model-value="cart.isAllSelected"
                                    @change="changeAllSelected(!cart.isAllSelected)">全选
                                </XtxCheckbox>
                            </th>
                    ...
                    <!-- 有效商品 -->
                    <tbody>
                        <tr v-for="item in cart.effectiveList" :key="item.id">
                            <td>
                                <XtxCheckbox :model-value="item.selected"
                                    @update:modelValue="changeSelected(item.skuId, $event)" />
                                <!--   <XtxCheckbox :model-value="item.selected"
                                    @change="changeSelected(item.skuId, !item.selected)" /> -->
                                <!-- 以下放权写法,违反了单项数据流 因为页面渲染的数据是由后端数据渲染而来的-->
                                <!-- <XtxCheckbox v-model='item.selected'
                                    @click='changeSelected(item.skuId, item.selected)' /> -->
                            </td>
                            <td>
                                <div class="goods">
                            ...
                            <td class="tc">
                                <!-- <XtxNumbox v-model='item.count' @click='changeCount(item.skuId, item.count)' /> -->
                                <XtxNumbox :model-value="item.count"
                                    @update:model-value="changeCount(item.skuId, $event)" :max="item.stock" />
                            </td>
                            <td class="tc">
                            ...
                            <td class="tc">
                                ...
                                <p><a class="green" href="javascript:;" @click="delCartItem([item.skuId])">删除</a>
                                </p>
                        ...
                        <tr v-if="cart.effectiveList.length === 0">
                            <td colspan="6">
                                <div class="cart-none" style="text-align: center;padding:100px 0 ;">
